...
'''
def UserName():
    user=input("entrez le nom d'utilisateur de votre base de donnée ")
    return user

def mdpBd():
        mdp=input("entrez le mot de passe de votre base de donnée ")
        return mdp

def CreatDataBase(mydb):
    cursordb = mydb.cursor()
    nomBase=input("Entrez le nom de la base de donnée ")
    requete="CREATE DATABASE {};".format(nomBase)
    cursordb.execute(requete)
'''
...
def creatTable(cursor):
    mycursordb = mydb.cursor()
    mycursor.execute("drop database")
    print("database created")
    requete="CREATE TABLE PremiereTable ( id int PRIMARY KEY, nom varchar (100),categorie varchar (100), prix float)"
    cursordb.execute(requete)


#Cette fonction renvoi le login le password et le salt

def RecupDonneFromUsersapp(mydb,login):
    mydb=mydb.cursor()
    requete="SELECT login,password,salt,statut FROM usersapp where login = %s "
    login=(login,)
    mydb.execute(requete,login)
    resultat=mydb.fetchall()
    if resultat:
        return resultat[0]
    else:
        return None
